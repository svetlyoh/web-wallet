...
When the historical checkpoint is far behind, the same hourly run also scans a separate 1,800-block window at the confirmed chain tip and persists verified words into the shared feed. This recent pass does not move the historical checkpoint or conceal an older gap. Inspect `scan.recent` in `/v1/stream` or `recent_scan` in `/v1/index-health` to verify its coverage. A complete recent pass can keep new words visible across browsers while the older history is repaired; `status: "unhealthy"` still signals the unresolved historical gap.
